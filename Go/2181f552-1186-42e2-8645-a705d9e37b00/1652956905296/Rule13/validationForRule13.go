	...
	import (
		...
		"strconv"
		"strings"
		parser "Organization1_Default_Department1_Default_Tenant1_validation/Go/2181f552-1186-42e2-8645-a705d9e37b00/1652956905296/Rule13/parser"
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewRule13Lexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewRule13Parser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
