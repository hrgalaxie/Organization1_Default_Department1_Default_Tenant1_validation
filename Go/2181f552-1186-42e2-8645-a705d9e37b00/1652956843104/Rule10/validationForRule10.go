	...
	import (
		...
		"strconv"
		"strings"
		parser "Organization1_Default_Department1_Default_Tenant1_validation/Go/2181f552-1186-42e2-8645-a705d9e37b00/1652956843104/Rule10/parser"
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		lexer:=parser.NewRule10Lexer(in)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
		...
		parser:=parser.NewRule10Parser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
