	...
	import (
		...
		"strconv"
		"strings"
		parser "Organization1_Default_Department1_Default_Tenant1_validation/Go/2181f552-1186-42e2-8645-a705d9e37b00/1652935938451/Rule3/parser"
...
expression :  age balance EOF ;

age : AGE ;

balance : BALANCE ;

AGE :SPLITKEY  OWNKEY [0-9]* ;

BALANCE :SPLITKEY  OWNKEY OWNKEY [0-9]* ;
...
`
	...
	func ValidateInput(input string,runID string,lineHashMap map[int]string) (bool,ErrorInfo){
		...
		parser:=parser.NewRule3Parser(stream)
		if outerMap, ok := ErrorHandleMap.Load(runID); ok {
			if value, canConvert := outerMap.(bool); canConvert {
